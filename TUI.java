...
public class TUI {
	...
	private Kwic kwic;  // Concordance viewer controller 
	private Scanner stdIn;
	
	public TUI(Kwic kwic) {
	...
	private void getAndProcessUserOption() {
		...
		case "kwic" : // KWIC view
			if (command.length == 1) {
				...
				if (command.length > 2) {
					...
					if (contextSize > 0) {
						display(kwic.kwicSearch(contextSize, word));
						break;
					}
					else {
				...
				display(kwic.kwicSearch(word));  // no valid context size given, assuming default context size...
			}
			break;
		case "detail" : // Wider context view
			if (command.length == 1) {
				...
				if (command.length == 2) {
					display(kwic.getResultsDetails(command[1]));
				}
				else {
